...
//	int random(void);
   ...
   double deltat;
	...
	fscanf(ifp,"%d %d %f %f %f %f %f %f %f %d", &nfile, &numrun, &noise, &q100, &q101, &q100_frac, &common_frac, &gain_frac, &readout_noise, &non_linear);
	fclose(ifp);
	printf("processing %d files starting from %d, noise = %f, threshold0 = %f, threshold1 = %f, rms threshold frac = %f, common_frac = %f, gain fraction = %f, readout noise = %f, nonlinear_resp = %d \n", numrun, nfile, noise, q100, q101, q100_frac, common_frac, gain_frac, readout_noise, non_linear);
